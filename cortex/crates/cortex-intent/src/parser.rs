...
fn fixme_pattern() -> &'static Regex {
    FIXME_PATTERN.get_or_init(|| {
        Regex::new(r"(?i)//\s*FIXME:\s*(.+)$").unwrap()
    })
}

fn ai_pattern() -> &'static Regex {
    AI_PATTERN.get_or_init(|| {
        Regex::new(r"(?i)//\s*AI:\s*(.+)$").unwrap()
    })
}
...
mod tests {
    ...
    fn test_parse_ai_comment() {
        ...
        assert!(matches!(intents[0].intent_type, IntentType::Generate { .. }));
    }
    ...
    fn test_parse_refactor() {
        ...
        assert_eq!(intents.len(), 1);
        assert!(matches!(intents[0].intent_type, IntentType::Refactor { .. }));
    }
