...
impl IntentRanker {
    ...
    pub fn compare(a: &Intent, b: &Intent) -> Ordering {
        ...
        let conf_cmp = b.confidence.partial_cmp(&a.confidence)
...
mod tests {
    ...
    fn test_confidence_ordering() {
        let high = make_intent(
            IntentType::Generate { prompt: "a".into(), context: vec![] },
            IntentSource::Chat,
            0.9,
        );
        let low = make_intent(
            IntentType::Generate { prompt: "b".into(), context: vec![] },
            IntentSource::Chat,
            0.5,
        );
    ...
    fn test_sort() {
        let mut intents = vec![
            make_intent(
                IntentType::Plan { description: "low".into() },
                IntentSource::Typing,
                0.5,
            ),
            make_intent(
                IntentType::Debug { error: "high".into(), context: vec![] },
