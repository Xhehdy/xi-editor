//! Cortex Vectors - Vector store abstraction for semantic search
//!
...
mod tests {
    ...
    async fn test_in_memory_store() {
        ...
        // Upsert
        store.upsert("vec1", vec![1.0, 0.0, 0.0], serde_json::json!({"label": "x"})).await.unwrap();
        store.upsert("vec2", vec![0.0, 1.0, 0.0], serde_json::json!({"label": "y"})).await.unwrap();
        store.upsert("vec3", vec![0.7, 0.7, 0.0], serde_json::json!({"label": "xy"})).await.unwrap();
