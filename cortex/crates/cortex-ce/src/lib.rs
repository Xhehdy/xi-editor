//! Cortex Comprehension Engine - Semantic understanding of codebases
//!
...
impl ComprehensionEngine {
    ...
    pub fn index_file(&mut self, path: &Path, content: &str) -> Result<Vec<SymbolId>, CEError> {
        ...
        // Track indexed file
        self.indexed_files.insert(path.to_path_buf(), symbol_ids.clone());
    ...
    fn extract_symbols(&self, path: &Path, content: &str, lang: cortex_syntax::Language) -> Result<Vec<Symbol>, CEError> {
        use cortex_syntax::SyntaxHighlighter;
...
fn is_definition_keyword(span: &cortex_syntax::HighlightSpan, content: &str) -> bool {
    let keyword = &content[span.start..span.end];
    matches!(keyword, "fn" | "struct" | "enum" | "trait" | "impl" | "const" | "static" | "type" | "mod")
}
