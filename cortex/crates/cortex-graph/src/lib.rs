//! Cortex Graph - Code Knowledge Graph with SQLite persistence
//!
...
impl CodeGraph {
    ...
    pub fn get_node(&self, id: NodeId) -> Result<Option<Node>, GraphError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, kind, name, file, data FROM nodes WHERE id = ?1"
        )?;

        let node = stmt.query_row([id.0], |row| {
            Ok(Node {
                id: NodeId(row.get(0)?),
    ...
    pub fn get_nodes_by_kind(&self, kind: NodeKind) -> Result<Vec<Node>, GraphError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, kind, name, file, data FROM nodes WHERE kind = ?1"
        )?;

        let nodes = stmt.query_map([kind.as_str()], |row| {
            Ok(Node {
                id: NodeId(row.get(0)?),
                kind: NodeKind::from_str(&row.get::<_, String>(1)?),
                name: row.get(2)?,
    ...
    pub fn get_outgoing(&self, from: NodeId) -> Result<Vec<Edge>, GraphError> {
        let mut stmt = self.conn.prepare(
            "SELECT from_id, to_id, kind, data FROM edges WHERE from_id = ?1"
        )?;

        let edges = stmt.query_map([from.0], |row| {
            Ok(Edge {
    ...
    pub fn get_incoming(&self, to: NodeId) -> Result<Vec<Edge>, GraphError> {
        let mut stmt = self.conn.prepare(
            "SELECT from_id, to_id, kind, data FROM edges WHERE to_id = ?1"
        )?;

        let edges = stmt.query_map([to.0], |row| {
            Ok(Edge {
                from: NodeId(row.get(0)?),
                to: NodeId(row.get(1)?),
                kind: EdgeKind::from_str(&row.get::<_, String>(2)?),
    ...
    pub fn get_neighbors(&self, from: NodeId, edge_kind: Option<EdgeKind>) -> Result<Vec<Node>, GraphError> {
    ...
    pub fn find_by_name(&self, name: &str) -> Result<Vec<Node>, GraphError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, kind, name, file, data FROM nodes WHERE name LIKE ?1"
        )?;

        let pattern = format!("%{}%", name);
        let nodes = stmt.query_map([pattern], |row| {
            Ok(Node {
                id: NodeId(row.get(0)?),
                kind: NodeKind::from_str(&row.get::<_, String>(1)?),
                name: row.get(2)?,
    ...
    pub fn get_nodes_in_file(&self, file: &str) -> Result<Vec<Node>, GraphError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, kind, name, file, data FROM nodes WHERE file = ?1"
        )?;

        let nodes = stmt.query_map([file], |row| {
            Ok(Node {
                id: NodeId(row.get(0)?),
                kind: NodeKind::from_str(&row.get::<_, String>(1)?),
                name: row.get(2)?,
...
mod tests {
    ...
    fn test_add_edge_and_get_neighbors() {
        ...
        let neighbors = graph.get_neighbors(NodeId(1), Some(EdgeKind::Calls)).unwrap();
