...
///
/// ```
/// # use glyph_rope::compare::sse_compare_mask;
...
pub unsafe fn ne_idx_avx(one: &[u8], two: &[u8]) -> Option<usize> {
    ...
    while idx < min_len {
        ...
        let mask = if stride_len == AVX_STRIDE {
            avx_compare_mask(
                one.get_unchecked(idx..idx + stride_len),
                two.get_unchecked(idx..idx + stride_len),
            )
        } else {
            let mut one_buf: [u8; AVX_STRIDE] = [0; AVX_STRIDE];
            let mut two_buf: [u8; AVX_STRIDE] = [0; AVX_STRIDE];
            one_buf[..stride_len].copy_from_slice(one.get_unchecked(idx..idx + stride_len));
            two_buf[..stride_len].copy_from_slice(two.get_unchecked(idx..idx + stride_len));
            avx_compare_mask(&one_buf, &two_buf)
        };
...
pub unsafe fn ne_idx_sse(one: &[u8], two: &[u8]) -> Option<usize> {
    ...
    while idx < min_len {
        ...
        let mask = if stride_len == SSE_STRIDE {
            sse_compare_mask(
                one.get_unchecked(idx..idx + stride_len),
                two.get_unchecked(idx..idx + stride_len),
            )
...
impl<'a> RopeScanner<'a> {
    ...
    ///
    /// ```
    /// # use glyph_rope::compare::RopeScanner;
    /// # use glyph_rope::Rope;
    ///
...
mod tests {
    ...
    fn make_lines(n: usize) -> String {
        ...
        let line: String = iter::repeat_n('a', 79).chain(iter::once('\n')).collect();
        for _ in 0..n {
            s.push_str(&line);
        }
        s
