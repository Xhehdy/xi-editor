...
enum RopeDeltaElementSerde {
    Copy(usize, usize),
    Insert(Node<RopeInfo>),
}

#[derive(Deserialize)]
struct RopeDeltaSerde {
    els: Vec<RopeDeltaElementSerde>,
    base_len: usize,
}

impl From<RopeDeltaElementSerde> for DeltaElement<RopeInfo> {
    fn from(elem: RopeDeltaElementSerde) -> DeltaElement<RopeInfo> {
        match elem {
            RopeDeltaElementSerde::Copy(start, end) => DeltaElement::Copy(start, end),
            RopeDeltaElementSerde::Insert(rope) => DeltaElement::Insert(rope),
        }
    }
}

impl From<RopeDeltaSerde> for Delta<RopeInfo> {
    fn from(mut delta: RopeDeltaSerde) -> Delta<RopeInfo> {
        Delta {
...
impl<'de> Deserialize<'de> for Delta<RopeInfo> {
        ...
        D: Deserializer<'de>,
    {
        ...
        let d = RopeDeltaSerde::deserialize(deserializer)?;
        Ok(Delta::from(d))
    }
}
